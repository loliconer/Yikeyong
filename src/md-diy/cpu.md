在选购CPU时，很多人只看主频。其实决定CPU的性能有多项指标，参考的性能参数有：插槽类型、主频、倍频、动态加速频率、核心数量、线程数量、工艺、缓存大小（一般指三级缓存）、总线技术、核心代号、指令集、总线规格、TDP（热设计功耗）。除了性能方面，还有其他几个方面也要关注，如：适用类型、支持最大内存、内存类型、是否带集成显卡、睿频加速技术、超线程技术、虚拟化技术等。
